...
{
    mRemoteObject = std::make_unique<JSONRPC::SmartLinkType<Core::JSON::IElement>>("SamplePlugin.1", "");
}
...
void JsonRpcFixture::DoEcho(::benchmark::State& state, const std::string& msg)
{
    ...
    uint32_t success = mRemoteObject->Invoke<EchoParamsData, Core::JSON::String>(5000, "echo", params, result);
