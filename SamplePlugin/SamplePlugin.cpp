...
namespace Plugin {
    ...
    const string SamplePlugin::Initialize(PluginHost::IShell* service)
    {
        ...
        if (_samplePlugin != nullptr) {
            ...
            // Spin up our own COM-RPC server
            SamplePluginConfiguration config;
            config.FromString(service->ConfigLine());

            if (config.PrivateComRpcServer) {
                _rpcEngine = Core::ProxyType<RPC::InvokeServer>::Create(&Core::IWorkerPool::Instance());
                _rpcServer = new ComRpcServer(Core::NodeId("/tmp/SamplePlugin"), _samplePlugin, service, service->ProxyStubPath(), _rpcEngine);

                if (_rpcServer->IsListening()) {
                    TRACE(Trace::Information, (_T("Successfully started COM-RPC server")));
                } else {
                    delete _rpcServer;
                    _rpcServer = nullptr;
                    _rpcEngine.Release();
                    TRACE(Trace::Error, (_T("Failed to start COM-RPC server")));

                    // Returning a string signals that we failed to initialize - WPEFramework will print this as an error message
                    success = "Failed to start COM-RPC server";
                }
            }
        } else {
            ...
            // Returning a string signals that we failed to initialize - WPEFramework will print this as an error message
            success = "Failed to initialize SamplePlugin";
        }
